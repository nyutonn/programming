...
-- 空部分のない行を見付ける
-- XXX 自分で書いて
findFullLine :: Box -> Maybe BoxPos
findFullLine box = Nothing

-- 一杯行を消す
collapseLine :: BoxPos -> Box -> Box
collapseLine = undefined
...
finish :: State -> Freer ()
finish st@State{..} = do
  getKey st (translate (-50) 0 $ scale 0.05 0.05 $
               text ("Finished: final score: " ++ show score))
  start randomg
...
okPlace pt Mino{mshape} State{box} =
  all (\mpt -> let p = point_add mpt pt in
               okPoint p && box ! p == emptyColor) mshape
...
drawCell pt = line [(x-boxWH,y-boxWH),
                    (x-boxWH,y+boxWH),
                    (x+boxWH,y+boxWH),
                    (x+boxWH,y-boxWH),
                    (x-boxWH,y-boxWH)]
...
{-
-- https://hackage.haskell.org/package/base-4.2.0.2/docs/src/Data-HashTable.html#hashInt
